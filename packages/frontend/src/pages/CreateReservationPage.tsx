 ...
 *   - Start date / End date (future dates only)
 *   - Payment method
 */
...
import type { CreateReservationResult } from '../types/api';
...
// Tomorrow's ISO date string (start must be at least tomorrow)
...
export default function CreateReservationPage() {
  ...
  // Loading state while fetching selectors
  if (dataLoading) {
    return (
  ...
  // Success state: show result summary
  if (result) {
    return (
      <div>
        <Link
        ...
        <div className="mt-6 bg-white rounded-lg shadow-sm p-6 max-w-lg">
          ...
          <dl className="mt-4 divide-y divide-gray-100 text-sm">
            <div className="flex justify-between py-2">
              ...
              <dd className="font-mono text-gray-900">{result.reservationId}</dd>
            </div>
            <div className="flex justify-between py-2">
              <dt className="text-gray-500">Equipment</dt>
              <dd className="text-gray-900">{result.equipmentName}</dd>
            </div>
            <div className="flex justify-between py-2">
            ...
            {result.discountApplied !== undefined && result.discountApplied > 0 && (
              <div className="flex justify-between py-2">
          ...
          <div className="mt-6 flex gap-3">
            <button
              type="button"
              onClick={() => navigate(`/reservations/${result.reservationId}`)}
  ...
  return (
    <div>
      ...
      <div className="mt-4">
        ...
        <p className="mt-1 text-sm text-gray-500">Reserve equipment for a future date range.</p>
      </div>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              >
                ...
                {(equipmentList ?? []).map((eq) => (
                  <option key={eq.equipmentId} value={eq.equipmentId}>
                    {eq.name} ({eq.category}) &mdash; ${eq.dailyRate}/day
                  </option>
                ))}
              </select>
              {errors.equipmentId && (
            ...
            <div>
              ...
              <input
                ...
                name="endDate"
                type="date"
                min={values.startDate ? tomorrowISO() > values.startDate
                  ? tomorrowISO()
                  : values.startDate
                  : tomorrowISO()}
